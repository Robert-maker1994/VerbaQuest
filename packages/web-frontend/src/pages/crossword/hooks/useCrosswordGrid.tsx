...
const isCellInWord = (cellRow: number, cellCol: number, word: WordData): boolean =>
  word.direction === "horizontal"
    ? cellRow === word.start_row && cellCol >= word.start_col && cellCol < word.start_col + word.word.length
    : cellCol === word.start_col && cellRow >= word.start_row && cellRow < word.start_row + word.word.length;
 ...
 */
export const useCrosswordGrid = ({ crosswordGrid, metadata }: UseCrosswordGridProps): UseCrosswordGridReturn => {
      ...
      for (let row = 0; row < crosswordGrid.length; row++) {
        for (let col = 0; col < crosswordGrid[row].length; col++) {
          ...
          if (crosswordGrid[row][col] !== "#") {
            const wordId = metadata.filter((word) => isCellInWord(row, col, word)).map((word) => word.word_id);
  ...
  const findAdjacentWord = useCallback(
    (currentWord: WordData, direction: "next" | "previous"): WordData | undefined => {
      const currentIndex = metadata.findIndex((w) => w.word_id === currentWord.word_id);
