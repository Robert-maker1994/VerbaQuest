...
export const useWordle = () => {
	...
	const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
		...
		if (event.key === "Enter") {
			...
			const updatedGuess =
				guesses[row].word.slice(0, col - 1) + guesses[row].word.slice(col);
			setGuesses(
				guesses.map((g, i) => (i === row ? { ...g, word: updatedGuess } : g)),
			);
			setActiveCell({ row, col: Math.max(0, col - 1) });
		} else if (/^[a-zA-Z]$/.test(event.key)) {
			const updatedGuess =
				guesses[row].word.slice(0, col) +
				event.key.toUpperCase() +
				guesses[row].word.slice(col + 1);
			setGuesses(
				guesses.map((g, i) => (i === row ? { ...g, word: updatedGuess } : g)),
			);
