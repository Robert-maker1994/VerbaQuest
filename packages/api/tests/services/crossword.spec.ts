...
import { crosswordService, ServiceError } from "../../libs/services/crossword";
...
describe("crosswordService", () => {
    ...
    it("should return crosswords by name", async () => {
        ...
        expect(client.select).toHaveBeenCalledWith(["c.title",
            "w.word_text",
            "cw.clue",
            "t.topic_name"
        ]);

        expect(client.where).toHaveBeenCalledWith("unaccent(Lower(t.topic_name)) ILike :name", {
            name: "%sample%",
        });
    ...
    it("should return crosswords by id", async () => {
        ...
        expect(client.where).not.toHaveBeenCalledWith("unaccent(Lower(t.topic_name)) ILike :name", {
            name: "%sample%",
        });
    ...
    it("should throw an error if the query fails", async () => {
        ...
        await expect(crosswordService({ name: "Sample" })).rejects.instanceOf(ServiceError);
    });
});
