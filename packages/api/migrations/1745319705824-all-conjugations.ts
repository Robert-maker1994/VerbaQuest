        ...
        for (const form of formEnity) {
            ...
            try {
                for (const [index, row] of csvData.entries()) {
                    ...
                    if (!formEntity) {
                        throw new Error(`Form with name '${form_name}', language '${verb_language_code}' not found in cache (Row ${index + 1})`);
                    }
                    ...
                    for (const key in row) {
                        if (key.startsWith("translation_") && row[key]) {
                            ...
                            if (!translationLanguage) {
                                console.warn(`Translation language '${translationLangCode}' not found for row ${index + 1}. Skipping translation.`);
                                continue;
                            }
