...
async function getAllCrosswordTopics() {
	try {
		...
		throw new CrosswordTopicError(
			`Error in getting crossword topics: ${err}`,
			500,
		);
	}
}
...
async function getCrosswordTopicByName(name: string) {
	try {
		...
		throw new CrosswordTopicError(
			`Error in getting crossword topics by name: ${err}`,
			500,
		);
	}
}
...
async function getCrosswordTopicById(id: number) {
	try {
		...
		if (err instanceof CrosswordTopicError) {
			throw new CrosswordTopicError(
				`Error in getting crossword topic by ID: ${err}`,
				500,
			);
		}
...
const createCrosswordService = async (data: CreateBody) => {
	...
	if (!language) {
		throw new LanguageError(
			"Unknown language please provide different language",
			200,
		);
	}
...
export {
	...
	createCrosswordService as createCrosswordTopicService,
};
