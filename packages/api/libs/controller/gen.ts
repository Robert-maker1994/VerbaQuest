 ...
 */
export function generateCrossword(words: string[]): [CrosswordGrid, CrosswordMetadata] {
    ...
    const crossword_grid: CrosswordGrid = Array(grid_size).fill(null).map(() => Array(grid_size).fill('#'));
    ...
    for (let word_index = 1; word_index < shuffled_words.length; word_index++) {
        ...
        for (const placed_word_info of placed_words_data) {
            for (let current_word_index = 0; current_word_index < current_word.length; current_word_index++) {
                const current_char: string = current_word[current_word_index];
                for (let placed_word_index = 0; placed_word_index < placed_word_info.word.length; placed_word_index++) {
                    ...
                    if (current_char === placed_char) {
                        ...
                        if (placed_word_info.direction === "vertical") {
                            const start_row_current: number = placed_word_info.start_row + placed_word_index;
                            const start_col_current: number = placed_word_info.start_col - current_word_index;
                            if (start_col_current >= 0 && (start_col_current + current_word.length <= grid_size)) {
                                let valid_horizontal = true;
                                for (let i_check = 0; i_check < current_word.length; i_check++) {
                                    const grid_char: string = crossword_grid[start_row_current][start_col_current + i_check];
                                    if (grid_char !== '#' && grid_char !== current_word[i_check]) {
                        ...
                        if (placed_word_info.direction === "horizontal") {
                            const start_row_current: number = placed_word_info.start_row - current_word_index;
                            const start_col_current: number = placed_word_info.start_col + placed_word_index;
                            if (start_row_current >= 0 && (start_row_current + current_word.length <= grid_size)) {
                                let valid_vertical = true;
                                for (let i_check = 0; i_check < current_word.length; i_check++) {
                                    const grid_char: string = crossword_grid[start_row_current + i_check][start_col_current];
                                    if (grid_char !== '#' && grid_char !== current_word[i_check]) {
        ...
        if (possible_placements.length > 0) {
            const chosen_placement = possible_placements[Math.floor(Math.random() * possible_placements.length)];
            const { direction, start_row, start_col } = chosen_placement;

            if (direction === "horizontal") {
                for (let i_place = 0; i_place < current_word.length; i_place++) {
                    crossword_grid[start_row][start_col + i_place] = current_word[i_place];
                }
            } else {
                for (let i_place = 0; i_place < current_word.length; i_place++) {
                    crossword_grid[start_row + i_place][start_col] = current_word[i_place];
                }
            }
