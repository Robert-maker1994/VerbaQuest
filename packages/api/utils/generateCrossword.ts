interface WordData {
   ...
   */
  function placeFirstWord(word: string, grid: CrosswordGrid, placedWords: WordData[]): void {
   ...
   */
  function placeWord(currentWord: string, crosswordGrid: CrosswordGrid, placedWordsData: WordData[]): void {
	  let possiblePlacements: PossiblePlacement[] = [];
	  ...
	  for (const placedWordInfo of placedWordsData) {
		  for (let currentWordIndex = 0; currentWordIndex < currentWord.length; currentWordIndex++) {
			  ...
			  for (let placedWordIndex = 0; placedWordIndex < placedWordInfo.word.length; placedWordIndex++) {
				  ...
				  if (currentChar === placedChar) {
					  ...
					  if (placedWordInfo.direction === "vertical") {
						  ...
						  if (isValidPlacement(crosswordGrid, currentWord, startRowCurrent, startColCurrent, "horizontal")) {
						  ...
						  if (isValidPlacement(crosswordGrid, currentWord, startRowCurrent, startColCurrent, "vertical"))
						  {
							  possiblePlacements.push({
								  direction: "vertical",
								  start_row: startRowCurrent,
  ...
  function removeEmptyRowsAndCols(grid: CrosswordGrid): CrosswordGrid {
	  ...
	  const newGrid = grid.slice(top, bottom + 1).map((row) => row.slice(left, right + 1));
	  return newGrid;
  }
   ...
   */
  function updateMetadata(grid: CrosswordGrid, metadata: CrosswordMetadata): CrosswordMetadata {
	  ...
	  for (const wordData of metadata.words_data) {
		...
		for (let row = 0; row < grid.length; row++) {
		  for (let col = 0; col < grid[row].length; col++) {
			if (grid[row][col] === wordData.word[0]) { // Check for the first letter of the word.
			  if (wordData.direction === "horizontal") { //horizontal
				let match = true;
				for (let i = 0; i < wordData.word.length; i++) {
				  if (col + i >= grid[row].length || grid[row][col + i] !== wordData.word[i]) {
					match = false;
					break;
				  }
				}
				...
				let match = true;
				for (let i = 0; i < wordData.word.length; i++) {
				  if (row + i >= grid.length || grid[row + i][col] !== wordData.word[i]) {
					match = false;
					break;
				  }
				}
